...
define($mainLang . '_MAIN', 'Main');
define($mainLang . '_ADDFROM', 'Alumni Listings');
define($mainLang . '_NEWTOD', 'new Alumni Listings have been added in this category today');
define($mainLang . '_NEWTRED', 'new Alumni Listings have been added in this category during the last 3 days');
...
define($mainLang . '_THE', 'The');
define($mainLang . '_LASTADD', 'Latest Alumni Listings');
define($mainLang . '_PREV', 'Previous Alumni Listings');
...
define($mainLang . '_GPHOTO', 'Graduation Photo');
define($mainLang . '_RPHOTO', 'Recent Photo');
define($mainLang . '_ADDED', 'added');
define($mainLang . '_GORUB', 'Go to ');
define($mainLang . '_DATE2', 'Alumni Listing added');
define($mainLang . '_DELETE', 'Delete');
...
define($mainLang . '_STOP3', '');
define($mainLang . '_VU', 'This has been viewed');
define($mainLang . '_VU2', 'times');
define($mainLang . '_OTHER', 'If your want to place another Listing ');
define($mainLang . '_THANK', 'Thank for using our Alumni Listing service.');
define($mainLang . '_TEAM', 'Team');
define($mainLang . '_ACTUALY', 'There are');
define($mainLang . '_ADVERTISEMENTS', 'Alumni Listing(s) active');
define($mainLang . '_MODADMIN', 'Change this Listings (administrator)');
define($mainLang . '_AND', 'and');
define($mainLang . '_WAIT3', 'Alumni Listing(s) waiting to be approved');
define($mainLang . '_CREATBY', 'was created by');
define($mainLang . '_FOR', 'for');
define($mainLang . '_OF', 'of');
define($mainLang . '_VIEW2', 'views');
define($mainLang . '_VIEWANN2', 'See your Listing');
define($mainLang . '_ACCESADMIN', 'Alumni Administration');
define($mainLang . '_NOANNINCAT', 'There are no Listings in this category');
define($mainLang . '_INCAT', 'in');
define($mainLang . '_SCHOOL2', 'School');
...
define($mainLang . '_JOBNOMODERATE', 'Your Alumni Listing will start immediately');
define($mainLang . '_FORMEMBERS2', 'Posting an Alumni Listing is restricted to members <br />Please register or login if you are already a member.');
...
define($mainLang . '_SELECTYPOTHER', 'Other School');
define($mainLang . '_SELECTCATOTHER', 'Other category');
define($mainLang . '_RETURN', 'Return');
define($mainLang . '_FILES', 'Files with this extension');
define($mainLang . '_FILESTOP', 'may not be uploaded. Upload stopped');
define($mainLang . '_YIMG', 'Your logo');
define($mainLang . '_TOBIG', 'is too big.<p> Use the function <b>Previous page</b> to return to the form an add a file');
define($mainLang . '_JOIND', 'Impossible to add this Listing');
define($mainLang . '_JOBADDED', 'Your Listing has been added.');
define($mainLang . '_PRINT', 'Print');
define($mainLang . '_FRIENDSEND', 'Send this Listing to a friend');
define($mainLang . '_IMGPISP', 'Photo available');
define($mainLang . '_VALIDERORMSG', 'There are errors which prevent this listing being saved!');
define($mainLang . '_VALIDNAME', 'First Name is required.');
define($mainLang . '_VALIDLNAME', 'Last Name is required.');
define($mainLang . '_VALIDCAT', 'You must select a category.');
define($mainLang . '_VALIDSCHOOL', 'School is required.');
define($mainLang . '_VALIDREQ', '');
define($mainLang . '_VALIDYEAR', 'Graduation Year is required.');
define($mainLang . '_VALIDEMAIL', 'Email is required.');
define($mainLang . '_VALIDSUBMITTER', 'Name is required.');
define($mainLang . '_VALIDMSG', 'Please correct these errors to save the listing.');

// fichier display-image.php //
define($mainLang . '_CLOSEF', 'Close this window');
...
define($mainLang . '_IP_LOGGED', ' and has been logged! Action will be taken on any abuse on this system.');
...
// message //
define($mainLang . '_CONTACTAFTERANN', 'Contact regarding your Alumni Listing');
define($mainLang . '_MESSFROM', 'Reply from');
define($mainLang . '_FROMANNOF', 'from the Alumni Section of {X_SITENAME}');
define($mainLang . '_REMINDANN', 'You have a reply to your Alumni Listing ');
define($mainLang . '_STARTMESS', '');
define($mainLang . '_ENDMESS', '');
define($mainLang . '_CANJOINT', 'You can reply to');
...
define($mainLang . '_SORTBY', 'Sort by:');
define($mainLang . '_CURSORTEDBY', 'Listings currently sorted by: %s');
define($mainLang . '_POPULARITYLTOM', 'Popularity (Least to Most Hits)');
define($mainLang . '_POPULARITYMTOL', 'Popularity (Most to Least Hits)');
//	define($mainLang.'_NAMEATOZ','Last Name (A to Z)');
//	define($mainLang.'_NAMEZTOA','Last Name (Z to A)');
define($mainLang . '_DATEOLD', 'Date (Old Listings First)');
define($mainLang . '_DATENEW', 'Date (New Listings First)');
...
define($mainLang . '_YOUR_LISTING', 'Your listing');
define($mainLang . '_AT', 'at');
define($mainLang . '_VEDIT_LISTING', 'View or edit your listing here');
define($mainLang . '_ALMOST', 'Your listing is about to expire');
define($mainLang . '_EXPIRED', 'has expired and has been deleted.');
define($mainLang . '_YOUR_LISTING_ON', 'Your listing on');
define($mainLang . '_ADDED_TO_CAT', 'A new listing has been added to the category ');
define($mainLang . '_RECIEVING_NOTIF', 'You have subscribed to receive notifications of this sort.');
define($mainLang . '_ERROR_NOTIF', 'If this is an error or you wish not to receive further such notifications, please update your subscriptions by visiting the link below:');
define($mainLang . '_FOLLOW_LINK', 'Here is a link to the new listing');

// ADDED FOR 3.01
define($mainLang . '_TITLEATOZ', 'Title (A to Z)');
define($mainLang . '_TITLEZTOA', 'Title (Z to A)');

define($mainLang . '_SEARCH_BYYEAR', 'By Year');
define($mainLang . '_SEARCH_KEYWORDS', 'Keywords');
...
define($mainLang . '_CAT', 'Category/School');
define($mainLang . '_ALUMNI', 'Alumni');
define($mainLang . '_SCHOOLATOZ', 'School (A to Z)');
define($mainLang . '_SCHOOLZTOA', 'School (Z to A)');
define($mainLang . '_YEAROLD', 'Class of (Older First)');
...
define($mainLang . '_SUBMIT', 'Submit');
define($mainLang . '_MODERATE', 'Your Alumni Listing will be queued for approval');
define($mainLang . '_NOMODERATE', 'Your Alumni Listing will start immediately');

define($mainLang . '_LISTING_NUM', 'Listing Number');

define($mainLang . '_NEW_GRAD_IMG', 'New Graduation Photo');
define($mainLang . '_NEW_NOW_IMG', 'New Recent Photo');
define($mainLang . '_NOW_IMG', 'Recent Photo');

define($mainLang . '_VALID', 'Show Publicly');
define($mainLang . '_DONT_PUBLISH', 'Don\t Publish');
define($mainLang . '_PUBLISH', 'Publish');

//define($mainLang . '_SEARCH', 'Search');
define($mainLang . '_PLZENTER', 'Please enter all required data!');
define($mainLang . '_SEARCHRESULTS', 'Search Results');
define($mainLang . '_NOMATCH', 'No Match Found for your Query');
//define('_MD_SEARCH_FOUND', 'Found <strong>%s</strong> match(es)');
define($mainLang . '_SHOWING', '(Showing %d - %d)');
define($mainLang . '_ANY', 'Any (OR)');
define($mainLang . '_ALL', 'All (AND)');
define($mainLang . '_EXACT', 'Exact Match');
define($mainLang . '_SHOWALLR', 'Show all results');
//define($mainLang . '_NEXT', 'Next >>');
define($mainLang . '_PREVIOUS', '<< Previous');
define($mainLang . '_KEYWORDS', 'Keyword(s)');
define($mainLang . '_TYPE', 'Type');
define($mainLang . '_SEARCHIN', 'Search in');
define($mainLang . '_KEYTOOSHORT', 'Keywords must be at least <strong>%s</strong> characters long');
define($mainLang . '_KEYIGNORE', 'Keywords shorter than <strong>%s</strong> characters will be ignored');
define($mainLang . '_SEARCHRULE', 'Search Rule');
define($mainLang . '_IGNOREDWORDS', 'The following words are shorter than allowed minmum length (%u chars) and were not included in your search:');

define($mainLang . '_FORMOK', 'Added successfully');
define($mainLang . '_FORMDELOK', 'Deleted successfully');
define($mainLang . '_FORMSUREDEL', 'Are you sure to delete it?');
define($mainLang . '_FORMUPLOAD', 'Upload Image');
...
define($mainLang . '_LISTINGLIST', 'List Alumni Listings');
define($mainLang . '_ERROR', 'Sorry, some error occurred!');

define($mainLang . '_APPROVE', 'Approve');

define($mainLang . '_LISTING_EDIT_MOD', 'Edit Alumni Listing<br />Your Listing will be queued for approval');
define($mainLang . '_ADD_LINK_MOD', 'Add an Alumni Listing<br />Your Listing will be queued for approval');

define($mainLang . '_SUBJECT', 'Subject');
define($mainLang . '_CONTACTALUMNI', 'Contact regarding your Alumni Listing');
define($mainLang . '_CONTACTADMIN', 'A reply to an Alumni Listing');
define($mainLang . '_ADMIN_COPY', 'This is a copy of the message sent through your system.');

define($mainLang . '_REPLYTO', 'You have a reply to your Alumni Listing ');
define($mainLang . '_FROMSITE', 'from the Alumni Section of {X_SITENAME}');
define($mainLang . '_CANREPLY', 'You can reply to');
