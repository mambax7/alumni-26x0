...
//        $form->display();
 //       break;
	...
	$cat_criteria->setStart($start);
        ...
        $cat_count = $alumni_categories_Handler->getCount($cat_criteria);
    ...
    $category_arr = $alumni_categories_Handler->getall($cat_criteria);
        ...
        if ($cat_count > 0) {
            ...
            foreach (array_keys($category_arr) as $i) {
                ...
                $perms = '';
                $groups_ids_view = $alumni_gperm_Handler->getGroupIds('alumni_view', $cid, $module_id);
    ...
    case 'update_view':
        ...
        if ($cid > 0 && $group > 0 && $status != '') {
            if ($status == 'no') {
                ...
                $alumni_gperm_Handler->deleteAll($criteria);
            } else {
                // add permissions
                $alumni_gperm_Handler->addRight('alumni_view', $cid, $group, $module_id);
            }
        }
        break;
}
