 ...
 */
define('ALUMNI_REQUEST_NOTRIM', 1);
define('ALUMNI_REQUEST_ALLOWRAW', 2);
define('ALUMNI_REQUEST_ALLOWHTML', 4);
...
class AlumniRequest
{
    ...
    static function getVar($name, $default = null, $hash = 'default', $type = 'none', $mask = 0)
    {
        ...
        if (isset($input[$name]) && $input[$name] !== null) {
            ...
            $var = AlumniRequest::_cleanVar($input[$name], $mask, $type);
            // Handle magic quotes compatability
            if (get_magic_quotes_gpc() && ($var != $default) && ($hash != 'FILES')) {
                $var = AlumniRequest::_stripSlashesRecursive($var);
            }
        } else if ($default !== null) {
            // Clean the default value
            $var = AlumniRequest::_cleanVar($default, $mask, $type);
        } else {
            $var = $default;
        }
        return $var;
    ...
    {
        return AlumniRequest::getVar($name, $default, $hash, 'int');
    }
    ...
    {
        return AlumniRequest::getVar($name, $default, $hash, 'float');
    }
    ...
    {
        return AlumniRequest::getVar($name, $default, $hash, 'bool');
    }
    ...
    {
        return AlumniRequest::getVar($name, $default, $hash, 'word');
    }
    ...
    {
        return AlumniRequest::getVar($name, $default, $hash, 'cmd');
    }
    ...
    static function getString($name, $default = '', $hash = 'default', $mask = 0)
    {
        ...
        return (string)AlumniRequest::getVar($name, $default, $hash, 'string', $mask);
    }

    static function getArray($name, $default = array(), $hash = 'default')
    {
        return AlumniRequest::getVar($name, $default, $hash, 'array');
    }

    static function getText($name, $default = '', $hash = 'default')
    {
        return (string)AlumniRequest::getVar($name, $default, $hash, 'string', ALUMNI_REQUEST_ALLOWRAW);
    }
    ...
    static function get($hash = 'default', $mask = 0)
    {
        ...
        $result = AlumniRequest::_cleanVar($input, $mask);
        // Handle magic quotes compatability
        if (get_magic_quotes_gpc() && ($hash != 'FILES')) {
            $result = AlumniRequest::_stripSlashesRecursive($result);
        }
        return $result;
    }
    ...
    static function set($array, $hash = 'default', $overwrite = true)
    {
        foreach ($array as $key => $value) {
            AlumniRequest::setVar($key, $value, $hash, $overwrite);
        }
    }
    ...
    {
        AlumniRequest::_cleanArray($_FILES);
        AlumniRequest::_cleanArray($_ENV);
        AlumniRequest::_cleanArray($_GET);
        AlumniRequest::_cleanArray($_POST);
        AlumniRequest::_cleanArray($_COOKIE);
        AlumniRequest::_cleanArray($_SERVER);
        if (isset($_SESSION)) {
            AlumniRequest::_cleanArray($_SESSION);
        }
        $REQUEST = $_REQUEST;
        $GET = $_GET;
        $POST = $_POST;
    ...
    static function _cleanVar($var, $mask = 0, $type = null)
    {
        ...
        if ($mask & 2) {
            ...
            if (is_null($safeHtmlFilter)) {
                $safeHtmlFilter = AlumniFilterInput::getInstance(null, null, 1, 1);
            }
            $var = $safeHtmlFilter->clean($var, $type);
        } else {
            // Since no allow flags were set, we will apply the most strict filter to the variable
            if (is_null($noHtmlFilter)) {
                $noHtmlFilter = AlumniFilterInput::getInstance( /* $tags, $attr, $tag_method, $attr_method, $xss_auto */);
            }
    ...
    {
        $value = is_array($value) ? array_map(array('AlumniRequest', '_stripSlashesRecursive'), $value) : stripslashes($value);
        return $value;
    }
}

/**
 * AlumniInput is a class for filtering input from any data source
 * Forked from the php input filter library by: Daniel Morris <dan@rootcube.com>
 * Original Contributors: Gianpaolo Racca, Ghislain Picard, Marco Wandschneider, Chris Tobin and Andrew Eddie.
 *
 * @author      Louis Landry <louis.landry@joomla.org>
 */
class AlumniFilterInput
{
     ...
     *      <pre>  $filter = & AlumniFilterInput::getInstance();</pre>
     *
     * @static
     *
     * @param   array   $tagsArray  list of user-defined tags
     ...
     * @param   int     $xssAuto    Only auto clean essentials = 0, Allow clean blacklisted tags/attr = 1
     *
     * @return  object  The AlumniFilterInput object.
     * @since   1.5
     */
    static  function getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1)
    {
        ...
        if (empty ($instances[$sig])) {
            $instances[$sig] = new AlumniFilterInput($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
        }
    ...
    public function clean($source, $type = 'string')
    {
        ...
        switch (strtoupper($type)) {
            ...
            case 'STRING' :
                ...
                if (isset($this) && is_a($this, 'AlumniFilterInput')) {
                    $filter =& $this;
                } else {
                    $filter = AlumniFilterInput::getInstance();
                }
            ...
            default :
                // Check for static usage and assign $filter the proper variable
                if (is_object($this) && get_class($this) == 'AlumniFilterInput') {
                    $filter =& $this;
                } else {
                    $filter = AlumniFilterInput::getInstance();
                }
    ...
    protected function _cleanAttributes($attrSet)
    {
        ...
        for ($i = 0; $i < count($attrSet); $i++) {
            ...
            if (AlumniFilterInput::checkAttribute($attrSubSet)) {
                continue;
            }
