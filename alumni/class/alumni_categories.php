...
//		$this->alumni = Alumni::getInstance();
	...
	$xoops = Xoops::getInstance();
		...
		$title = $this->isNew() ? sprintf(_AM_ALUMNI_CATEGORY_ADD) : sprintf(_AM_ALUMNI_CATEGORY_EDIT);
		...
		$form->setExtra('enctype="multipart/form-data"');
			...
			$form->addElement(new Xoops\Form\Label(_AM_ALUMNI_CATEGORY_PID, $mytree->makeSelBox("pid", "title","-", $this->getVar("pid"),true)));
		$form->addElement(new Xoops\Form\Text(_AM_ALUMNI_CATEGORY_TITLE, "title", 50, 255, $this->getVar("title")), true);
		//$form->addElement(new XoopsFormTextArea(_AM_TEST1_CATEGORY_DESC, "category_desc", $this->getVar("category_desc"), 4, 47), true);
			...
			$uploadirectory_category_img = '/modules/alumni/images/cat';
			$imgtray_category_img = new Xoops\Form\ElementTray(_AM_ALUMNI_IMGCAT,'<br />');
			$imgpath_category_img = sprintf(_AM_ALUMNI_FORMIMAGE_PATH, $uploadirectory_category_img);
			...
			$fileseltray_category_img->addElement(new Xoops\Form\File(_AM_ALUMNI_FORMUPLOAD , "img", $xoops->getModuleConfig("alumni_photomax")),false);
		...
		$form->addElement(new Xoops\Form\Text(_AM_ALUMNI_ORDER, "ordre", 50, 255, $this->getVar("ordre")), false);
		$form->addElement(new Xoops\Form\Label(_AM_ALUMNI_IFSCHOOL,''));	

			$photo_old = $this->getVar("scphoto") ? $this->getVar("scphoto") : '';
			$uploadirectory_photo = XOOPS_ROOT_PATH."/modules/alumni/photos/school_photos";
			$imgtray_photo = new Xoops\Form\ElementTray(_AM_ALUMNI_SCPHOTO,'<br />');
			$imgpath_photo = sprintf(_AM_ALUMNI_FORMIMAGE_PATH, $uploadirectory_photo);
			$fileseltray_photo = new Xoops\Form\ElementTray('','<br />');
			$fileseltray_photo->addElement(new XoopsFormFile(_AM_ALUMNI_FORMUPLOAD , "scphoto", $xoops->getModuleConfig("alumni_photomax")),false);
 			if ($photo_old) {
			$fileseltray_photo->addElement(new Xoops\Form\Label(_AM_ALUMNI_ACTUALPICT,'<a href="../photos/school_photos/'.$photo_old.'">'.$photo_old.'</a>', false));
		...
		$form->addElement(new Xoops\Form\Text(_AM_ALUMNI_SCADDRESS, "scaddress", 50, 255, $this->getVar("scaddress")), false);	
		$form->addElement(new Xoops\Form\Text(_AM_ALUMNI_SCADDRESS2, "scaddress2", 50, 255, $this->getVar("scaddress2")), false);	
		$form->addElement(new Xoops\Form\Text(_AM_ALUMNI_SCCITY, "sccity", 50, 255, $this->getVar("sccity")), false);	
		$form->addElement(new Xoops\Form\Text(_AM_ALUMNI_SCSTATE, "scstate", 50, 255, $this->getVar("scstate")), false);	
		$form->addElement(new Xoops\Form\Text(_AM_ALUMNI_SCZIP, "sczip", 50, 255, $this->getVar("sczip")), false);	
		$form->addElement(new Xoops\Form\Text(_AM_ALUMNI_SCPHONE, "scphone", 50, 255, $this->getVar("scphone")), false);	
		$form->addElement(new Xoops\Form\Text(_AM_ALUMNI_SCFAX, "scfax", 50, 255, $this->getVar("scfax")), false);
		$form->addElement(new Xoops\Form\Text(_AM_ALUMNI_SCMOTTO, "scmotto", 50, 255, $this->getVar("scmotto")), false);	
		$form->addElement(new Xoops\Form\Text(_AM_ALUMNI_SCURL, "scurl", 50, 255, $this->getVar("scurl")), false);	
